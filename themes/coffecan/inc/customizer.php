 ...
 */
function coffecan_customize_register( $wp_customize ) {
    ...
    $wp_customize->add_control(
        new WP_Customize_Color_Control(
            ...
            'theme_bg_color',
            array(
                'label'		=> __( 'Header and footer background color', 'coffecan'),
                'section'	=> 'colors',
    ...
    $wp_customize->add_control(
        new WP_Customize_Color_Control(
            ...
            'interactive_color', array(
                'label'		=> __( 'Interactive color (links etc)', 'coffecan' ),
                'section'	=> 'colors',
    ...
    $wp_customize->add_section( 'theme_options',
        array(
            'title'			=> __( 'Theme Options', 'coffecan' ),
            'priority'		=> 95,
            'capability'	=> 'edit_theme_options',
            'description'	=> __( 'Change how much of a post is displayed on index and archive pages.', 'coffecan' )
        )
    );
    ...
    $wp_customize->add_setting(	'length_setting',
        array(
            'default'			=> 'excerpt',
            'type'				=> 'theme_mod',
            'sanitize_callback' => 'coffecan_sanitize_length', // Sanitization function appears further down
            'transport'			=> 'postMessage'
        )
    );

    // Add the controls
    $wp_customize->add_control(	'coffecan_length_control',
        array(
            'type'		=> 'radio',
            'label'		=> __( 'Index/archive displays', 'coffecan' ),
            'section'	=> 'theme_options',
            'choices'	=> array(
                'excerpt'		=> __( 'Excerpt (default)', 'coffecan' ),
                'full-content'	=> __( 'Full content', 'coffecan' )
            ),
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial( 'blogname', array(
			...
			'render_callback' => 'coffecan_customize_partial_blogname',
		) );
		$wp_customize->selective_refresh->add_partial( 'blogdescription', array(
			'selector'        => '.site-description',
			'render_callback' => 'coffecan_customize_partial_blogdescription',
		) );
	}
}
add_action( 'customize_register', 'coffecan_customize_register' );
 ...
 * @return void
 */
function coffecan_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 * @return void
 */
function coffecan_customize_partial_blogdescription() {
 ...
 */
function coffecan_customize_preview_js() {
	wp_enqueue_script( 'coffecan-customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'coffecan_customize_preview_js' );
...
function coffecan_sanitize_length( $value ) {
...
if ( ! function_exists( 'coffecan_header_style' ) ) :
    /**
     * Styles the header image and text displayed on the blog.
     *
     * @see coffecan_custom_header_setup().
     */
    function coffecan_header_style() {
